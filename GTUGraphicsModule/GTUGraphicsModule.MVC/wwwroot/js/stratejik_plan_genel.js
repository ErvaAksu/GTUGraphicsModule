...
function exportToExcel() {
    ...
    var lineData = [];
    var lineDataset = lineChart.data.datasets[0];
    ...
    var polarData = [{ "Yıl": currentYear }];
    var polarDataset = polarChart.data.datasets[0];
    polarDataset.data.forEach(function (value, i) {
        var polarPoint = {

            Kod: polarChart.data.labels[i],
            Değer: value
        };
...
function initializeLineChart(lineData) {
    ...
    yearDataMap.get(currentYear).yVals.forEach((data) => {
    ...
    lineCtx.canvas.addEventListener('click', function (event) {
        let activePoints = lineChart.getElementsAtEventForMode(event, 'point', lineChart.options);
        if (activePoints.length > 0) {
            ...
            polarChart.data.labels = [];
            polarChart.update();

            

            if (clickedData != 0) {
                currentYear = clicked_label;
                yearDataMap.get(clicked_label).yVals.forEach((data) => {
                    polarChart.data.datasets[0].data.push(data);
                });
