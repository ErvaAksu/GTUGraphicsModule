...
function exportChartToExcelBirim() {
    ...
    var tableData = [{ "Yıl": selectedYearBirim }];
    barChartArrayBirim.forEach(function (barChart, index) {
        ...
        options: {
            ...
            onClick: function (event, elements) {
                if (elements && elements.length > 0) {
                    ...
                    updateTableRowsBirim(canvasId, clickedData, head, kod);
...
function updateBirimBarChart(canvasId, barData) {
    ...
    chartToUpdate = barChartArrayBirim[canvasId];
...
function initializeTableBirim(canvasxId, barData) {
...
function updateTableRowsBirim(index, data, tanim, kod) {

    let tableCaption = document.getElementById('birim-table-heading-' + index);
    tableCaption.textContent = kod + ' - ' + tanim;

    let tableBody = document.getElementById('birim-table-body-' + index);
    tableBody.innerHTML = '';
    ...
    let colorParts = document.querySelectorAll('.color-inner-selection-birim');

    updateColorsAndScalesBirim(index, colorParts);
}


function updateColorsAndScalesBirim(index, colorParts) {
    ...
    for (let i = 0; i < tableBody.rows.length; i++) {
        ...
        let color = getColorForThresholdBirim(gerceklesmeOrani, colorParts);

        if (color) {

            row.style.backgroundColor = color;
            row.style.color = getTextColorBasedOnBackgroundColorBirim(color);
...
function getColorForThresholdBirim(value, colorParts) {
    for (let i = 0; i < colorParts.length; i++) {
        let threshold = parseInt(colorParts[i].querySelector('.threshold').value, 10);
        let color = colorParts[i].querySelector('.color-part-birim input').value;
