...
function exportChartToExcel() {
    ...
    var tableData = [{ "Yıl": selectedYear }];
    barChartArray.forEach(function (barChart, index) {
        ...
        options: {
            ...
            onClick: function (event, elements) {
                if (elements && elements.length > 0) {
                    ...
                    updateTableRows(canvasId, clickedData, head, kod);
...
function initializeTable(canvasxId, barData) {
...
function updateTableRows(index, data, tanim, kod) {
    
    let tableCaption = document.getElementById('table-heading-'+index);
    tableCaption.textContent = kod + ' - ' + tanim ;

    let tableBody = document.getElementById('table-body-' + index);
    tableBody.innerHTML = ''; 
    ...
    let colorParts = document.querySelectorAll('.color-inner-selection'); 

    updateColorsAndScales(index, colorParts);
}


function updateColorsAndScales(index, colorParts) {
    ...
    for (let i = 0; i < tableBody.rows.length; i++) {
        ...
        let color = getColorForThreshold(gerceklesmeOrani, colorParts);

        if (color) {

            row.style.backgroundColor = color;
            row.style.color = getTextColorBasedOnBackgroundColor(color);
...
function getColorForThreshold(value, colorParts) {
    for (let i = 0; i < colorParts.length; i++) {
        let threshold = parseInt(colorParts[i].querySelector('.threshold').value, 10);
        let color = colorParts[i].querySelector('.color-part input').value;
...
function getTextColorBasedOnBackgroundColor(bgColor) {
