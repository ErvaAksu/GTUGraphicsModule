...
function exportToExcelKonu() {
    ...
    var lineData = [];
    var lineDataset = lineChartKonu.data.datasets[0];
    ...
    var polarData = [{ "Yıl": currentYearKonu }];
    var polarDataset = polarChartKonu.data.datasets[0];
    polarDataset.data.forEach(function (value, i) {
        var polarPoint = {

            Kod: polarChartKonu.data.labels[i],
            Değer: value
        };
...
function initializeKonuLineChart(lineData) {

    let currentDate = new Date();

    currentYearKonu = currentDate.getFullYear();
    ...
    let lineCtx = document.getElementById('line-chart-konu').getContext('2d');

    // Create the line chart
    lineChartKonu = new Chart(lineCtx, {
        type: 'line',
        data: lineChartData,
    ...
    yearDataMap.get(currentYearKonu).yVals.forEach((data) => {
    ...
    let polarCtx = document.getElementById('polar-chart-konu').getContext('2d');

    // Create or update the polar area chart
    polarChartKonu = new Chart(polarCtx, {
        type: 'polarArea',
        data: polarChartData,
    ...
    lineCtx.canvas.addEventListener('click', function (event) {
        let activePoints = lineChartKonu.getElementsAtEventForMode(event, 'point', lineChartKonu.options);
        if (activePoints.length > 0) {
            ...
            polarChartKonu.data.datasets[0].data = [];
            polarChartKonu.data.labels = [];
            polarChartKonu.update();


            if (clickedData != 0) {
                currentYearKonu = clicked_label;
                yearDataMap.get(clicked_label).yVals.forEach((data) => {
                    polarChartKonu.data.datasets[0].data.push(data);
                });
